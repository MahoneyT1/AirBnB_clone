...
env.key_filename = os.getenv('my_ssh_private_key')
...
def do_clean(number=0):
    """Checks the total number of archive files in the version folder
    args:
        number is the total number to keep and delete others
        ...
        Delete all unnecessary archives (all archives minus the number
        to keep) in the versions folder Delete all unnecessary archives
        (all archives minus the number to keep) in the /data/web_static
        /releases folder of both of your web servers
    """
    if os.path.exists('/'):
        list_of_folders = os.listdir('/versions')
